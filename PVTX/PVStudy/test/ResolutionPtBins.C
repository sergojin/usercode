...
void ResolutionPtBins()
{
  //gROOT->SetStyle("CMS");
  gStyle->SetOptFit(1111);
  gStyle->SetFillColor(0);

  TH1F* dx = new TH1F("h1","h1",200,-0.1,0.1);
  TH1F* dy = new TH1F("h2","h2",200,-0.1,0.1);
  TH1F* dz = new TH1F("h3","h3",200,-0.1,0.1);

  files["Data2011A_MinBias"]   = TFile::Open("/uscms_data/d3/sergo/crab_MB7TEV_Aug05_v3/res/ntuple_merged_Run2011A_Express.root");
  ...
  for (map<string,TFile*>::const_iterator ifile = files.begin(); ifile != files.end(); ifile++) {
	...
	TTree* t = (TTree*)  ifile->second->Get(TString("pvtxtree"));
	...
	for (Int_t ev = 0; ev < t->GetEntries()/100.0; ev++) {
	  
	  t->GetEntry(t->GetEntries()-ev-1);

	  
	  for (int iv=0; iv<nrecPV_twovtx; ++iv){  

	    if ((nTrkPV_twovtx[iv]==ntrk)&&(avgPtPV1_spl_twovtx[iv]>2.0)){

	    dx->Fill( deltax_twovtx[iv]);
	    dy->Fill( deltay_twovtx[iv]);
	    dz->Fill( deltaz_twovtx[iv]);
	    }
	...
	h["gresx_"+tag1_+tag_+suffix] = (TH1*) dx->Clone();
	h["gresy_"+tag1_+tag_+suffix] = (TH1*) dy->Clone();
	h["gresz_"+tag1_+tag_+suffix] = (TH1*) dz->Clone();
	h["gpullx_"+tag1_+tag_+suffix] = (TH1*) dx->Clone();
	h["gpully_"+tag1_+tag_+suffix] = (TH1*) dy->Clone();
	h["gpullz_"+tag1_+tag_+suffix] = (TH1*) dz->Clone();
	h["gerrResx_"+tag1_+tag_+suffix] = (TH1*) dx->Clone();
	h["gerrResy_"+tag1_+tag_+suffix] = (TH1*) dy->Clone();
	h["gerrResz_"+tag1_+tag_+suffix] = (TH1*) dz->Clone();
...
void FillHistos(string type, vector<string> sampleNames, vector<string> pvNames,
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if(type == "Res") {
      tex1_[i]=new TLatex(5, 360, "CMS Preliminary 2010, #sqrt{s}=7 TeV");
      tex1_[i]->SetTextSize(0.04);   
      //tex2_[i]=new TLatex(6.5, 245, "#sqrt{s}=7 TeV");
      //tex2_[i]->SetTextSize(0.04);
    }
    else if(type == "Pull") {
      tex1_[i]=new TLatex(4, 1.8, "CMS Preliminary 2010, #sqrt{s}=7 TeV");
      tex1_[i]->SetTextSize(0.04);   
