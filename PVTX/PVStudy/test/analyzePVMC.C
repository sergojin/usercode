...
void analyzePVMC()
{
  ...
  files["Pythia6_QCD"]   = TFile::Open("/uscms_data/d3/sergo/crab_QCDMC_Pythia6_TuneZ2_test/res/histo_Pythia_merged.root");
  ...
  TFile *rootFile_ = new TFile("Summary_SAMPLE.root","RECREATE");
  rootFile_->cd();
...
void FillHistos(string type, vector<string> sampleNames, vector<string> pvNames,
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if(type == "Res") {
      tex1_[i]=new TLatex(5, 360, "CMS Preliminary 2011, #sqrt{s}=ENERGY");
      tex1_[i]->SetTextSize(0.04);   
      //tex2_[i]=new TLatex(6.5, 245, "#sqrt{s}=ENERGY");
      //tex2_[i]->SetTextSize(0.04);
    }
    else if(type == "Pull") {
      tex1_[i]=new TLatex(5, 1.8, "CMS Preliminary 2011, #sqrt{s}=ENERGY");
      tex1_[i]->SetTextSize(0.04);   
      //tex2_[i]=new TLatex(6, 1.6, "#sqrt{s}=ENERGY");
      //tex2_[i]->SetTextSize(0.04);
    }
 
    legend_[i]=new TLegend(0.3,0.6,0.85,0.85);
    legend_[i]->SetTextSize(0.03);  
  ...
  for (int i = 0; i < 3; i++) {
    ...
    //tex2_[i]->Draw();
    canvas_[i]->Write();
    canvas_[i]->Print(TString("EPSDIR/SAMPLE/"+type+coord[i]+".eps"));
    canvas_[i]->Print(TString("PNGDIR/SAMPLE/"+type+coord[i]+".png"));
  }
}
