...
def main():
    ...
    if len(sys.argv) < 4:
        print "\n [Usage] python SubmitJobs.py <JobsList.txt> <caf/lxplus/cmslpc> <local/batch>"
        sys.exit()

    cfidir = "../../python/"
    cfifilepath = "UserCode.PVStudy."
...
##            print tagName
            ...
            cfiname = cfidir+"PVStudy_"+tagName+"_cfi.py"
            fout = open(cfiname,"w")
            ...
            cfinametag = cfifilepath+"PVStudy_"+tagName+"_cfi"
            #print cfinametag
            lumirange = d[3:len(d)-1]+":"+str(lumilist[j*2])+"-"+d[3:len(d)-1]+":"+str(lumilist[j*2+1])
            #print lumirange
            treeoutfile = d+"pvtxana_ntuple_"+tagName+".root"
            #print treeoutfile
            histoutfile = d+"pvtxana_hist_"+tagName+".root"
            #print treeoutfile
            replacetag = [
                ('INPUT_FILE',cfinametag),
                ('LUMIRANGE',lumirange),
                ('OUTFILE1',treeoutfile),
                ('OUTFILE2',histoutfile)
                ]

            tmpcfgfile = open("Step1_template_cfg.py")
            cfgtagname = "Step1_"+tagName+"_cfg.py"
            ...
            if jobmode == "local":
                runjobcmd = "cmsRun "+d+cfgtagname+" >& "+d+"PVStudy_"+tagName+".log &"
                print runjobcmd
                os.system(runjobcmd)
...
##                print workdir
                runlogtag = d+"PVStudy_"+tagName+".log"
                replacetag1 = [('PWD',workdir),
                               ('CFGFILE',cfgtagname),
                               ('CFGRUNLOG',runlogtag)
                               ]
                tmpbjobfile = open("SubmitJobs_template.sh","r")
                bjobName = "SubmitJobs_"+tagName+".sh"
