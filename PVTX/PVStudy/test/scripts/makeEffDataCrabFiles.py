...
cmsswSkelFile = 'pvres_Data_cfg.py'
dataSet = ''
lumimask = ''
nLumis = -1
nLumisPerJob = 25
...
report_every = 1000;
global_tag_flag = '';
crabdir = '/uscms_data/d1/ygao/pvtx7TeV/eff/'
ptCut = '0'
...
def makeCrabConfig():
    ...
    if(str(ptCut) != '0'):
        outFileName = outFileName + '_ptCut' + ptCut;
    outFile = open(outFileName + '_eff.cfg', 'w')
    print 'Writing CRAB config file: ' + outFileName + '_eff.cfg'
    outFile.write('[CRAB]\n')
    outFile.write('jobtype   = cmssw\n')
    outFile.write('scheduler = ' + mode + '\n')
    outFile.write('\n[CMSSW]\n')
    outFile.write('datasetpath             = ' + dataSet + '\n')
    outFile.write('pset                    = ' + outFileName + '_eff_cfg.py \n')
...
def makeCMSSWConfig(cmsswSkelFile):
    ...
    ptcut = 0.5
    if(str(ptCut) == '0'):
        outFileName = dataSet.split('/')[1]+'_'+dataSet.split('/')[2] + '_fullRange_eff_cfg.py'
        ptcut = 0.0;
    if(str(ptCut) == '1'):
        outFileName = dataSet.split('/')[1]+'_'+dataSet.split('/')[2] + '_ptCut1_eff_cfg.py'
        ptcut = 0.5;        
    if(str(ptCut) == '2'):
        outFileName = dataSet.split('/')[1]+'_'+dataSet.split('/')[2] + '_ptCut2_eff_cfg.py'
        ptcut = 0.9;
    ...
    for i in inFile:
        ...
        if i.find('process.pveffana.ptcut') != -1:
            outFile.write('process.pveffana.ptcut = ' + str(ptcut) + '\n'); continue
...
if len(sys.argv) < 5 :
    ...
    print '\t-ptCut\t\tptCut. Default is 0 corresponding to no pT cut'
    sys.exit()
